...
const deleteOldImages = async (product, newBanner, newImages) => {
...
router.post(
	...
	async (req, res) => {
		try {
			...
			// Process banner
			if (images.find((img) => img.type === 'banner')) {
				productData.banner = images.find((img) => img.type === 'banner').url;
			}
...
router.delete('/:id/images', auth, isManagerOrAdmin, async (req, res) => {
	try {
		const { id } = req.params;
...
router.get('/on-sale/all', async (req, res) => {
	try {
		const { limit = 10 } = req.query;
...
router.post('/:id/stock', auth, isManagerOrAdmin, async (req, res) => {
	try {
		const { id } = req.params;
